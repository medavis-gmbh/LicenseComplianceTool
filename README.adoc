...
`failOnDynamicLicense` only sees licenses that are named but cannot be resolved. A component that declares *no* license at all passes silently, is listed in the
component manifest without a license and has no license text shipped for it. Set `failOnMissingLicense` to fail on those as well. The message names the affected
components, so they can be given a license through the component metadata - or excluded from the manifest with `ignore` if they are not third-party software.
...
pipeline {
    ...
    stages {
        stage('Create manifest') {
            steps {
                downloadLicenses inputPath: 'bom.json', outputPath: 'target/THIRDPARTY', failOnDynamicLicense: true, failOnMissingLicense: true, configurationProfile: 'MyProfile'
            }
        }
    }
}
...
----
* Download licenses and fail on anything unresolved - a license that is not in the configuration, or a component that declares no license at all
[source]
----
  java -jar license-compliance-tool-cli.jar download-licenses --in=path/to/bom --out=manifest.html --fail-dynamic-license --fail-missing-license
----
* Get usage help
[source]
----
