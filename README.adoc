...
Specifies a location within the file system where license files are stored so that they do not have to be downloaded each time.
The path is relative to the working directory of the Jenkins process.
Note that the location is relative to the node on which the `downloadLicenses` build step is executed, so it is shared between different jobs, but not across
build nodes.

If left empty, caching will be disabled and licenses will be downloaded each run.
...
pipeline {
    ...
    stages {
        stage('Create manifest') {
            steps {
                componentManifest inputPath: 'input.json', outputPath: 'output.pdf'
            }
        }
    }
}
...
=== Download licenses
This build step tries to downloads all licenses referenced in the input SBOM and the global configuration. nto the specified directory.
Download URLs are preferred over view URLs. If no URL has been specified for the license, then nothing will be downloaded.
